...
export default function usePortal({ isOpen: defaultIsOpen = false, closeOnEsc = true, closeOnOutsideClick = true }: UsePortalOptions = {}) {
  ...
  const openPortal = useCallback(() => {
    if (targetEl.current === null) {
      throw Error("Для открытия портала, необходилмо добавить ref для элемента");
    }
    setOpen(true);
  }, [setOpen, targetEl]);
  ...
  const handleOutsideMouseDownClick = useCallback((e: MouseEvent): void => {
